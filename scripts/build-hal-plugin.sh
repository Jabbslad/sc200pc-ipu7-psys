...
# Intel's ipu7-camera-hal.
#
# The SC200PC DOL2 enablement lives on the fork branch 'sc200pc-dol2',
# based on Intel's April release tag (ef307675 / 20260406_1900_297) plus the
# project's DOL2 commits. We pin the exact source commit so the build is
# reproducible from source; the resulting binary hash is environment-specific
# and will only match the reference build on the reference machine.
#
# Prerequisites: cmake, a C++ toolchain, and the Intel IPU7 camera dev
# dependencies (AIQ/CCA/AIC headers and libraries from ipu7-camera-bins, plus
# libdrm, jsoncpp, etc. as the HAL's CMake requires).
#
# Usage: scripts/build-hal-plugin.sh
set -euo pipefail
...
install -D -m 0755 -- "$so" "$ARTIFACTS/ipu75xa.so"

hash=$(sha256sum "$ARTIFACTS/ipu75xa.so" | awk '{print $1}')
echo "==> built $ARTIFACTS/ipu75xa.so"
echo "    sha256: $hash"
if [[ $hash == "$REFERENCE_PLUGIN_SHA256" ]]; then
    ...
    echo "    (expected on a different toolchain/libs; source is pinned to $PINNED_HAL_COMMIT)"
fi
echo "==> next: install using your built plugin's hash:"
echo "    sudo PLUGIN_SHA256=$hash scripts/sc200pc-apply.sh apply"
