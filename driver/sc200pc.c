...
#define SC200PC_REG_DOL_DIRECTION	0x3802
...
#define SC200PC_REG_SHORT_EXPOSURE_H	0x3e22
#define SC200PC_REG_SHORT_EXPOSURE_M	0x3e04
#define SC200PC_REG_SHORT_EXPOSURE_L	0x3e05
#define SC200PC_REG_ANALOGUE_GAIN_H	0x3e08
#define SC200PC_REG_ANALOGUE_GAIN_L	0x3e09
#define SC200PC_REG_SHORT_GAIN_H		0x3e12
#define SC200PC_REG_SHORT_GAIN_L		0x3e13
#define SC200PC_REG_DOL_LONG_LINES_H	0x3e23
#define SC200PC_REG_DOL_LONG_LINES_L	0x3e24
#define SC200PC_REG_BLACK_LEVEL		0x3901

/* Intel CRL control ID consumed by the IPU7 HAL for DOL short exposure. */
...
/*
 * The real sc200pc.sys mode record reports 832000000 bits/s per lane on a
 * two-lane D-PHY link. V4L2_CID_LINK_FREQ describes the D-PHY clock, so its
 * value is half the per-lane bit rate because data is transferred on both
 * clock edges.
 */
...
#define SC200PC_NUM_STREAMS		2
#define SC200PC_STREAM_MASK		GENMASK_ULL(SC200PC_NUM_STREAMS - 1, 0)
#define SC200PC_HTS			0x047e	/* 1150 internal clocks/line */
#define SC200PC_VTS_DEF			0x08d4	/* 2260 lines/frame */
#define SC200PC_VTS_MIN			SC200PC_VTS_DEF
#define SC200PC_VTS_MAX			0x7ff0

/*
 * The SC200PC uses internal column-parallel readout: multiple pixels are
 * sampled per internal clock cycle, so HTS (1150) < active width (1928), and
 * DOL2 spends two hardware line periods for each requested exposure line.
 * V4L2 describes either 1088-line VC, not their aggregate DOL geometry, and
 * computes: llp = HBLANK + width, fll = VBLANK + height.
 *
 * We therefore report a per-VC FLL of 1130 and an LLP of 4×HTS. This keeps
 * the pixel rate and frame-duration arithmetic exact while giving AIQ the
 * same line budget consumed by the recovered DOL equation:
 *   llp = 4*1150 = 4600,  HBLANK = 4600 − 1928 = 2672
 *   fll = 2260/2 = 1130,  VBLANK = 1130 − 1088 = 42
 *   pixel_rate = 4600 × 1130 × 30 = 155 940 000
 *   frame_dur  = 4600 × 1130 / 155940000 = 33.33 ms  (30 fps)
 *
 * cur_vts remains in hardware units; VBLANK and exposure controls use the
 * per-VC request-line units advertised to userspace.
 */
#define SC200PC_INTERNAL_PARALLELISM	2
#define SC200PC_DOL_LINES_PER_EXPOSURE	2
#define SC200PC_FLL_DEF			(SC200PC_VTS_DEF / SC200PC_DOL_LINES_PER_EXPOSURE)
#define SC200PC_FLL_MIN			(SC200PC_VTS_MIN / SC200PC_DOL_LINES_PER_EXPOSURE)
#define SC200PC_FLL_MAX			(SC200PC_VTS_MAX / SC200PC_DOL_LINES_PER_EXPOSURE)
#define SC200PC_LLP			(SC200PC_HTS * SC200PC_INTERNAL_PARALLELISM * \
					 SC200PC_DOL_LINES_PER_EXPOSURE)
#define SC200PC_HBLANK_DEF		(SC200PC_LLP - SC200PC_WIDTH)
#define SC200PC_VBLANK_DEF		(SC200PC_FLL_DEF - SC200PC_HEIGHT)
#define SC200PC_PIXEL_RATE_DEFAULT	((u64)SC200PC_LLP * SC200PC_FLL_DEF * SC200PC_FPS)

#define SC200PC_EXPOSURE_MIN		1
#define SC200PC_EXPOSURE_MAX_MARGIN	8
#define SC200PC_EXPOSURE_MAX		(SC200PC_FLL_DEF - SC200PC_EXPOSURE_MAX_MARGIN)
#define SC200PC_EXPOSURE_DEFAULT	SC200PC_EXPOSURE_MAX
#define SC200PC_SHORT_EXPOSURE_DEFAULT	1
#define SC200PC_ANALOGUE_GAIN_MIN	0x10
#define SC200PC_ANALOGUE_GAIN_MAX	0xf8	/* 15.5x after OEM encoding */
#define SC200PC_ANALOGUE_GAIN_DEFAULT	0x10

#define SC200PC_DIGITAL_GAIN_MIN	128
#define SC200PC_DIGITAL_GAIN_MAX	255
...
struct sc200pc {
	...
	struct v4l2_ctrl *short_exposure;
...
static int sc200pc_write_regs(struct sc200pc *sensor, u16 reg, u32 val,
			      unsigned int len)
{
	struct i2c_client *client = sensor->client;
	u8 buf[5] = { reg >> 8, reg & 0xff };
	unsigned int i;
	int ret;
...
{
	return sc200pc_write_regs(sensor, reg, val, 1);
}
...
static u32 sc200pc_exposure_max(const struct sc200pc *sensor)
{
	return max_t(u32, SC200PC_EXPOSURE_MIN,
		     sensor->cur_vts / SC200PC_DOL_LINES_PER_EXPOSURE -
		     SC200PC_EXPOSURE_MAX_MARGIN);
}
...
static void sc200pc_update_exposure_range(struct sc200pc *sensor)
{
	...
	__v4l2_ctrl_modify_range(sensor->short_exposure,
				 SC200PC_EXPOSURE_MIN, max_exp, 1,
				 SC200PC_SHORT_EXPOSURE_DEFAULT);
}

struct sc200pc_dol_timing {
	u32 frame;
	u32 long_lines;
	u32 long_q;
	u32 short_q;
...
/*
 * Reproduce the SC200PC DOL2 timing contract recovered from the OEM driver.
 * Exposure requests are in lines; the DOL integration registers use four
 * times that value. All arithmetic below is an integer form of the recovered
 * ratio equation, avoiding floating point in the kernel.
 */
static void sc200pc_calc_dol_timing(struct sc200pc *sensor, u32 long_exp,
				    u32 short_exp,
				    struct sc200pc_dol_timing *timing)
{
	u32 frame;
	u32 long_limit;
	u32 short_limit;
	...
	long_exp = max_t(u32, long_exp, SC200PC_EXPOSURE_MIN);
	short_exp = max_t(u32, short_exp, SC200PC_EXPOSURE_MIN);

	frame = 2 * long_exp + 2 * short_exp + 0xb;
	frame = clamp_t(u32, frame, sensor->cur_vts, SC200PC_VTS_MAX);
	frame = ALIGN(frame, 2);

	/*
	 * floor(((2 * frame - 13) + 9 * long/short) /
	 *       (2 * long/short + 2)), then force the result even.
	 */
	numerator = (u64)(2 * frame - 0xd) * short_exp +
		    (u64)9 * long_exp;
	denominator = (u64)2 * (long_exp + short_exp);
	timing->long_lines = div64_u64(numerator, denominator) & ~1U;
	timing->long_lines = max_t(u32, timing->long_lines, 0x10);

	long_limit = (frame - timing->long_lines) * 2 - 0xd;
	short_limit = timing->long_lines * 2 - 9;
	timing->long_q = clamp_t(u32, 4 * long_exp, 2, long_limit);
	timing->short_q = clamp_t(u32, 4 * short_exp, 2, short_limit);

	/* Preserve the OEM driver's transition-parity rule. */
	if (sensor->prev_long_q &&
	    ((abs((int)sensor->prev_long_q - (int)timing->long_q) & 1) != 0))
		timing->long_q++;
	if (sensor->prev_short_q &&
	    ((abs((int)sensor->prev_short_q - (int)timing->short_q) & 1) != 0))
		timing->short_q++;

	/*
	 * Avoid the split-point glitch observed by the OEM implementation when
	 * long_lines moves down too far in one update.
	 */
	if (timing->long_lines < sensor->prev_long_lines &&
	    frame - 0x8b8 < sensor->prev_long_lines - timing->long_lines) {
		frame += sensor->prev_long_lines - timing->long_lines -
			 (frame - 0x8b8);
		frame = ALIGN(frame, 2);
	}

	timing->frame = min_t(u32, frame, SC200PC_VTS_MAX);
	timing->direction = !sensor->prev_long_lines ||
			    timing->long_lines <= sensor->prev_long_lines;
}
...
static void sc200pc_log_key_regs(struct sc200pc *sensor, const char *tag)
{
	static const struct {
		...
		{ SC200PC_REG_ANALOGUE_GAIN_L, "3e09" },
...
static int sc200pc_set_dol_exposures(struct sc200pc *sensor, u32 long_exp,
				     u32 short_exp)
{
	struct sc200pc_dol_timing timing;
	int ret;

	long_exp = clamp_t(u32, long_exp,
			   SC200PC_EXPOSURE_MIN, sc200pc_exposure_max(sensor));
	short_exp = clamp_t(u32, short_exp,
			    SC200PC_EXPOSURE_MIN, sc200pc_exposure_max(sensor));
	sc200pc_calc_dol_timing(sensor, long_exp, short_exp, &timing);

	/* The OEM sequence commits the DOL split point before the other fields. */
	ret = sc200pc_write_reg(sensor, SC200PC_REG_GROUP_HOLD, 0x00);
	if (ret)
		return ret;

	ret = sc200pc_write_regs(sensor, SC200PC_REG_DOL_LONG_LINES_H,
				timing.long_lines, 2);
	if (ret) {
		sc200pc_write_reg(sensor, SC200PC_REG_GROUP_HOLD, 0x30);
		return ret;
	}

	ret = sc200pc_write_reg(sensor, SC200PC_REG_GROUP_HOLD, 0x30);
	if (ret)
		return ret;

	ret = sc200pc_write_reg(sensor, SC200PC_REG_DOL_DIRECTION,
			       timing.direction);
	if (ret)
		return ret;

	ret = sc200pc_write_regs(sensor, SC200PC_REG_VTS_H, timing.frame, 2);
	if (ret)
		return ret;

	ret = sc200pc_write_regs(sensor, SC200PC_REG_EXPOSURE_H,
				(timing.long_q & 0xffff) << 4, 3);
	if (ret)
		return ret;

	ret = sc200pc_write_reg(sensor, SC200PC_REG_SHORT_EXPOSURE_H,
			       (timing.short_q >> 12) & 0x0f);
	if (ret)
		return ret;

	ret = sc200pc_write_regs(sensor, SC200PC_REG_SHORT_EXPOSURE_M,
				(timing.short_q & 0x0fff) << 4, 2);
	if (ret)
		return ret;

	sensor->prev_long_lines = timing.long_lines;
	sensor->prev_long_q = timing.long_q;
	sensor->prev_short_q = timing.short_q;

	return 0;
}

static u16 sc200pc_encode_analogue_gain(u32 gain)
{
	if (gain < 0x10)
		return 0x0010;
	if (gain < 0x20)
		return gain;
	if (gain < 0x40)
		return 0x0100 | (gain >> 1);
	if (gain < 0x80)
		return 0x0300 | (gain >> 2);
	if (gain < 0xf9)
		return 0x0700 | (gain >> 3);
	return 0x071f;
}

static int sc200pc_set_analogue_gain(struct sc200pc *sensor, u32 gain)
{
	u16 encoded = sc200pc_encode_analogue_gain(gain);
	int ret;

	/* DOL2 uses the same gain request for the long and short exposures. */
	ret = sc200pc_write_regs(sensor, SC200PC_REG_ANALOGUE_GAIN_H,
				encoded, 2);
	if (!ret)
		ret = sc200pc_write_regs(sensor, SC200PC_REG_SHORT_GAIN_H,
					encoded, 2);
...
static int sc200pc_set_digital_gain(struct sc200pc *sensor, u32 gain)
{
	...
	/* The recovered DOL2 path has one shared digital-gain byte. */
	return sc200pc_write_reg(sensor, SC200PC_REG_DIGITAL_GAIN_L, gain);
}
...
static int sc200pc_apply_controls(struct sc200pc *sensor)
{
	...
	ret = sc200pc_set_dol_exposures(sensor, sensor->exposure->val,
				       sensor->short_exposure->val);
	if (ret)
		return ret;

	ret = sc200pc_set_analogue_gain(sensor, sensor->analogue_gain->val);
	if (ret)
		return ret;

	return sc200pc_set_digital_gain(sensor, sensor->digital_gain->val);
}
...
static int sc200pc_set_vts(struct sc200pc *sensor, u16 vts)
{
	...
	ret = sc200pc_write_regs(sensor, SC200PC_REG_VTS_H, vts, 2);
	if (ret)
		return ret;
...
static int sc200pc_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VBLANK: {
		...
		/* The following long-exposure write applies VTS with both DOL fields. */
		sensor->cur_vts = vts;
	...
	switch (ctrl->id) {
	case CRL_CID_EXPOSURE_SHS1:
		/* Stage short first; the HAL writes long next to commit the pair. */
		ret = 0;
		break;
	case V4L2_CID_EXPOSURE:
		ret = sc200pc_set_dol_exposures(sensor, ctrl->val,
					       sensor->short_exposure->val);
		break;
...
static const struct v4l2_ctrl_config sc200pc_short_exposure_ctrl = {
	.ops = &sc200pc_ctrl_ops,
	.id = CRL_CID_EXPOSURE_SHS1,
	.name = "DOL Short Exposure",
	.type = V4L2_CTRL_TYPE_INTEGER,
	.min = SC200PC_EXPOSURE_MIN,
	.max = SC200PC_EXPOSURE_MAX,
	.step = 1,
	.def = SC200PC_SHORT_EXPOSURE_DEFAULT,
};
...
static int sc200pc_start_streaming(struct sc200pc *sensor)
{
	...
	sensor->prev_long_lines = 0;
	sensor->prev_long_q = 0;
	sensor->prev_short_q = 0;
...
static int sc200pc_enable_streams(struct v4l2_subdev *sd,
				  ...
				  u32 pad, u64 streams_mask)
{
	...
	if (pad || !streams_mask || (streams_mask & ~SC200PC_STREAM_MASK))
		return -EINVAL;

	mutex_lock(&sensor->lock);

	if (sensor->enabled_streams & streams_mask) {
		ret = -EALREADY;
		goto out_unlock;
	}

	if (!sensor->enabled_streams) {
	...
	sensor->enabled_streams |= streams_mask;
	dev_dbg(sensor->dev, "enabled streams %#llx (active %#llx)\n",
		streams_mask, sensor->enabled_streams);
...
static int sc200pc_disable_streams(struct v4l2_subdev *sd,
				   struct v4l2_subdev_state *state,
				   u32 pad, u64 streams_mask)
{
	struct sc200pc *sensor = to_sc200pc(sd);
	...
	if ((sensor->enabled_streams & streams_mask) != streams_mask) {
		ret = -EALREADY;
		goto out_unlock;
	}
	...
	sensor->enabled_streams &= ~streams_mask;
	if (!sensor->enabled_streams) {
		/* Keep core stream bookkeeping synchronized if stream-off I2C fails. */
		sc200pc_stop_streaming(sensor);
		sc200pc_power_off(sensor);
		sensor->streaming = false;
	}

	dev_dbg(sensor->dev, "disabled streams %#llx (active %#llx)\n",
		streams_mask, sensor->enabled_streams);
			   ...
			   struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->pad || fmt->stream >= SC200PC_NUM_STREAMS)
		return -EINVAL;

	state_fmt = v4l2_subdev_state_get_format(state, fmt->pad, fmt->stream);
	if (!state_fmt)
		return -EINVAL;

	fmt->format = *state_fmt;
	return 0;
}
			   ...
			   struct v4l2_subdev_format *fmt)
{
	...
	state_fmt->width = SC200PC_WIDTH;
	state_fmt->height = SC200PC_HEIGHT;
	state_fmt->code = MEDIA_BUS_FMT_SBGGR10_1X10;
	state_fmt->field = V4L2_FIELD_NONE;
	state_fmt->colorspace = V4L2_COLORSPACE_RAW;

	fmt->format = *state_fmt;
	return 0;
}

static int sc200pc_get_frame_desc(struct v4l2_subdev *sd, unsigned int pad,
				  struct v4l2_mbus_frame_desc *desc)
{
	...
	desc->num_entries = SC200PC_NUM_STREAMS;

	for (stream = 0; stream < SC200PC_NUM_STREAMS; stream++) {
		desc->entry[stream].stream = stream;
		desc->entry[stream].pixelcode = MEDIA_BUS_FMT_SBGGR10_1X10;
		desc->entry[stream].bus.csi2.vc = stream;
		desc->entry[stream].bus.csi2.dt = MIPI_CSI2_DT_RAW10;
	}

	return 0;
}

static int sc200pc_set_routing(struct v4l2_subdev *sd,
			       struct v4l2_subdev_state *state,
			       ...
			       struct v4l2_subdev_krouting *routing)
{
	...
	struct v4l2_subdev_route routes[SC200PC_NUM_STREAMS] = {
		{
			.source_pad = 0,
			.source_stream = 0,
	...
	struct v4l2_subdev_krouting fixed_routing = {
		.num_routes = ARRAY_SIZE(routes),
		.routes = routes,
	};

	/* The sensor always emits the long exposure on VC0 and short on VC1. */
	return v4l2_subdev_set_routing_with_fmt(sd, state, &fixed_routing,
						 &format);
}

static int sc200pc_init_state(struct v4l2_subdev *sd,
			      struct v4l2_subdev_state *state)
{
	return sc200pc_set_routing(sd, state, V4L2_SUBDEV_FORMAT_ACTIVE, NULL);
}

static const struct v4l2_subdev_internal_ops sc200pc_internal_ops = {
	.init_state = sc200pc_init_state,
};
...
static const struct v4l2_subdev_pad_ops sc200pc_pad_ops = {
	...
	.get_frame_desc = sc200pc_get_frame_desc,
	.set_routing = sc200pc_set_routing,
	.enable_streams = sc200pc_enable_streams,
	.disable_streams = sc200pc_disable_streams,
};
...
static int sc200pc_probe(struct i2c_client *client)
{
	...
	sensor->sd.internal_ops = &sc200pc_internal_ops;
	...
	ret = v4l2_ctrl_handler_init(&sensor->ctrls, 12);
	if (ret)
	...
	sensor->vblank = v4l2_ctrl_new_std(&sensor->ctrls, &sc200pc_ctrl_ops,
					   V4L2_CID_VBLANK,
					   SC200PC_FLL_MIN - SC200PC_HEIGHT,
					   SC200PC_FLL_MAX - SC200PC_HEIGHT,
	 ...
	 * PLL). See the per-VC DOL timing comment above.
	 */
	...
	if (sensor->exposure)
		sensor->exposure->flags |= V4L2_CTRL_FLAG_EXECUTE_ON_WRITE;

	sensor->short_exposure =
		v4l2_ctrl_new_custom(&sensor->ctrls,
				     &sc200pc_short_exposure_ctrl, NULL);
